...
📋 Temel Özellikler Delta Senkronizasyonu: date_gte parametresi sayesinde sadece en son çalıştırılmadan sonra eklenen yeni domainleri çeker.
...
🚀 Kurulum Gereksinimler: Node.js (v16 veya üzeri) yüklü olmalıdır.

Paketleri Yükleyin:

Bash npm install axios 💻 Kullanım Talimatları ## Tam Senkronizasyon (Fetch + Upload) Sistemi ilk kez çalıştırıyorsanız veya tüm güncellemeleri çekip yüklemek istiyorsanız:

Bash node index.js Bu komut önce **API**'den yeni domainleri çeker, ardından Cisco Umbrella'ya yükler.


## Sadece Yükleme (Upload - --skip-fetch)
...
Bash node index.js --skip-fetch 📂 Dosya Yapısı index.js: Ana otomasyon scripti.
...
⚙️ Yapılandırma index.js dosyasının üst kısmındaki değişkenleri kendi ortamınıza göre güncelleyin:
...
🔧 Hata Giderme *No such file* hatası: Scripti ilk kez normal (parametresiz) modda çalıştırdığınızdan emin olun; dosyalar ilk çalıştırmada oluşturulur.
